...
static void server_uart_rx_callback(const void *buffer, uint16_t length, bool error)
{
    ...
    if (((char *)buffer)[0] == 0xD) {
        uapi_uart_write(0, (uint8_t *)"\r\n", (uint16_t)strlen("\r\n"), 0);
    } else {
        uapi_uart_write(0, (const uint8_t *)buffer, (uint32_t)length, 0);
    }
    ret = uapi_at_channel_data_recv(0, (uint8_t *)buffer, (uint32_t)length);
    if (ret != 0) {
        /* 前一个字符为'\r'时单独一个'\n'导致的CHANNEL_BUSY不打印 */
    ...
    SleAcbStateType conn_state, SlePairStateType pair_state, SleDiscReasonType disc_reason)
{
    ...
    if (conn_state == OH_SLE_ACB_STATE_CONNECTED) {
        ...
        (void)uapi_uart_register_rx_callback(0, UART_RX_CONDITION_FULL_OR_SUFFICIENT_DATA_OR_IDLE,
                                         16, server_uart_rx_callback);
